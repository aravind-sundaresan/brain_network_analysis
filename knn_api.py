...
class KNN:
    ...
    def train(self, X_train, X_test):
        ...
        for i in range(len(X_test)):
            euclidean_distance_matrix.append(self.compute_euclidean_distance(X_train, X_test[i]))

        return euclidean_distance_matrix

    def predict(self, X_test, euclidean_distance_matrix, y_train):
        y_train = y_train.flatten()
        output_labels = []

        for i in range(len(X_test)):
                nearest_neighbours = self.find_knn(euclidean_distance_matrix[i])
                neighbour_labels = []

                for neighbour in nearest_neighbours:
                        neighbour_labels.append(y_train[neighbour])

                #neighbour_labels = np.array(neighbour_labels).flatten()
